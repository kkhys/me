...
const MetadataCodeBlock = ({ post }: { post: Post }) => {
  ...
  const tagObjectList = post.tagObjectList as
    | (typeof allTags)[number][]
    | undefined;
        ...
        <React.Fragment key={outerIndex}>
          ...
          {Object.entries(object).map(([key, value], innerIndex) => (
            <span key={innerIndex}>
              ...
              <span className='text-[#5de4c7]'>{value}</span>
              <DoubleQuote />
              {Object.entries(object).length !== innerIndex + 1 && (
                <span className='text-[#a6accd]'>,</span>
              )}
            </span>
          ))}
          <span>
            <span className='text-[#a6accd]'>{'    }'}</span>
            {objectArray.length !== outerIndex + 1 && (
              <span className='text-[#a6accd]'>,</span>
            )}
          </span>
        </React.Fragment>
    ...
    <div className='max-h-[450px] overflow-x-auto rounded-md bg-[#131316] p-6 shadow-md dark:border [&_span]:h-4'>
      <pre>
        <code className='grid gap-1.5 text-sm text-muted-foreground [&_span]:h-4'>
          ...
          <KeyValuePair
            propName='publishedAtFormattedUs'
            value={publishedAtFormattedUs}
          />
          <KeyValuePair
            propName='publishedAtFormattedIso'
            value={publishedAtFormattedIso}
          />
          <KeyValuePair propName='updatedAt' value={updatedAt} />
          <KeyValuePair
            propName='updatedAtFormattedUs'
            value={updatedAtFormattedUs}
          />
          <KeyValuePair
            propName='updatedAtFormattedIso'
            value={updatedAtFormattedIso}
          />
          ...
          <KeyValuePair
            propName='revisionHistoryUrl'
            value={revisionHistoryUrl}
          />
          <KeyValuePairObject
            keyName='categoryObject'
            object={categoryObject}
          />
          <KeyValuePairObjectArray
            keyName='tagObjectList'
            objectArray={tagObjectList}
          />
...
const ConfigAction = ({ post }: { post: Post }) => {
        ...
        <DialogContent className='sm:max-w-[625px]'>
          <DialogHeader>
            ...
            <DialogDescription className='font-sans'>
              This metadata consists of the metadata automatically set by
              Contentlayer and my own implementation of Frontmatter in the MDX
              file. Generated by SSG.
            </DialogDescription>
          </DialogHeader>
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle className='text-left font-serif'>Metadata</DrawerTitle>
          <DrawerDescription className='text-left font-sans'>
            This metadata consists of the metadata automatically set by
            Contentlayer and my own implementation of Frontmatter in the MDX
            file. Generated by SSG.
          </DrawerDescription>
