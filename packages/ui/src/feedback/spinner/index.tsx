...
const Spinner = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<'div'>>(
  ({ className, ...props }, ref) => {
    const computeDelay = (i: number) => `${-1.2 + i * 0.1}s`;
    const computeRotation = (i: number) => `${i * 30}deg`;

    return (
      <div className={cn('size-5', className)} role='status' aria-label='Loading' ref={ref} {...props}>
