...
const MobileNavigationItem = ({ href, children, isExternal = false }: NavigationItemProps) => {
          ...
          >
            ...
            <nav className='mt-6'>
              <ul className='-my-2 divide-y divide-zinc-100 text-base text-zinc-800 dark:divide-zinc-100/5 dark:text-zinc-300'>
                <MobileNavigationItem href='https://github.com/users/kkhys/projects/1' isExternal>
                  Roadmap
...
const NavigationItem = ({ href, children, isExternal = false }: NavigationItemProps) => {
  const segment = useSelectedLayoutSegment();
  let isActive = segment === href;
  const styles = {
    text: clsx(
  ...
  return (
    <li>
      {isExternal ? (
        <a href={href} target='_blank' rel='noreferrer noopener' className={styles.text}>
          {children}
        </a>
      ) : (
  ...
  return (
    <nav {...props}>
      <ul className='flex rounded-full bg-white/90 px-3 text-sm font-medium text-zinc-800 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10'>
        <NavigationItem href='https://github.com/users/kkhys/projects/1/views/1' isExternal>
          Roadmap
        </NavigationItem>
      </ul>
    </nav>
...
const AvatarContainer = ({ className, ...props }: JSX.IntrinsicElements['div']) => {
  return (
    <div
      className={clsx(
        className,
...
export const Header = () => {
  ...
  useEffect(() => {
    ...
    const updateHeaderStyles = () => {
      let top = headerRef.current?.getBoundingClientRect().top ?? 0;
      let height = headerRef.current?.getBoundingClientRect().height ?? 0;
